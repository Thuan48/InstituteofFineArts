...
const ListSubmission = () => {
  ...
  const handleDeleteFromExhibition = (submissionId) => {
    const exhibitionSubmission = exhibitionSubmissions.find(es => es.SubmissionId === submissionId && es.ExhibitionId === competitionId)
  ...
  const isInExhibition = (submissionId) => {
    return exhibitionSubmissions.some(es => es.SubmissionId === submissionId && es.ExhibitionId === competitionId)
  }
        ...
        <Grid container spacing={2}>
          {competitionSubmissions.map((sub) => (
            <Grid item xs={12} sm={6} md={4} key={sub.submissionId}>
              <Card
                style={{ marginBottom: "1rem", cursor: "pointer", border: selectedSubmissions.includes(sub.submissionId) ? '2px solid blue' : 'none' }}
